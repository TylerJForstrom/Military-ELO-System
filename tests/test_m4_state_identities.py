...
class StateCodePolicyTests(unittest.TestCase):
    ...
    def test_every_policy_window_targets_an_existing_seed_identity(self) -> None:
        ...
        for code, windows in SEED_CODE_POLICIES.items():
            for _, _, entity_id in windows:
                with self.subTest(code=code, entity=entity_id):
...
class IdentityDenyWindowTests(unittest.TestCase):
    def test_turkey_deny_window_blocks_every_tier(self) -> None:
        ...
        for low, high in ((1921, 1921), (1919, 1920), (1923, 1925)):
...
class CuratedExclusionTableTests(unittest.TestCase):
    def test_exclusion_tables_are_enumerated_and_documented(self) -> None:
        self.assertEqual(len(HCED_CURATED_EXCLUSIONS), 10)
        self.assertEqual(len(HCED_LABEL_CURATED_EXCLUSIONS), 8)
        self.assertEqual(set(IWD_CURATED_PARENT_EXCLUSIONS), {"10", "42"})
...
class TrancheReleaseArtifactTests(unittest.TestCase):
    ...
    def test_ledger_composition_pins(self) -> None:
        self.assertEqual(len(self.events), 4341)
        ...
        self.assertEqual(len(label), 2274)
        self.assertEqual(len(crosswalk), 1798)
        self.assertEqual(sum(str(e["id"]).startswith("iwd_war_") for e in self.events), 55)
        self.assertEqual(sum(str(e["id"]).startswith("iwbd_") for e in self.events), 167)
