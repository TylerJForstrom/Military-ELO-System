...
The polity registry and rating ledger are separate products. The current expanded provisional release catalogues 1,590 time-bounded polity identities, but only 226 have accepted rating evidence. Its 4,341 events consist of 40 manually curated events, 1,798 crosswalk-resolved HCED tactical encounters, 2,274 label-resolved HCED tactical encounters, 55 coalition-aggregated IWD strategic parent wars, 167 IWBD tactical battles, and 7 UCDP conflict-termination strategic episodes. The review queues hold 27,083 staged source records, of which 23,459 are event-like candidates; 19,125 of those remain outside the rating ledger. An unrated registry entry has no Elo result; it is not assigned a baseline score, loss, or implied lack of military success.
...
HCED rows lacking Seshat coding on one or both sides are not discarded; they are retried in a second, declared label-resolution pass. The two passes are strictly ordered — the coded pass, then IWD aggregation, then the label pass — so the crosswalk-resolved events and the IWD aggregation are provably unchanged by the second pass. A deferred side resolves only through an explicit time-bounded label policy or an exact-normalized alias match, with none of the coded pass's guarantees weakened: exact normalized equality, exactly one match, and full validity over the entire event date range; any other cardinality fails. The label-policy tables are authoritative: a label with a policy entry must fit fully inside exactly one declared window, and outside or between windows it fails with no fallback to the alias tiers — the gaps (for example France 1816–1851 and 1941–1957, or Russia 1918–1921) are deliberate, so events there stay staged rather than being attached to a neighboring identity. Faction and collective-peoples labels are blocklisted ahead of every alias tier and can never resolve, even when a source record carries a colliding alias — promoting one later requires an explicit policy entry mapping it to a curated identity, which is how seven former faction labels (royalists, parliamentarians, taiping, carlists, chinese communists, spanish nationalists, spanish republicans) migrated off the blocklist to curated non-state actor identities, joining five actor labels that were never blocklisted — and genuine polity names whose only time-valid identity is a multi-regime envelope are quarantined pending curated identity splits. Declared identity deny windows are enforced in every label-resolution pipeline (the label "Turkey" never resolves for events intersecting 1920-1923), and enumerated curated row exclusions stage known wrong-actor and variant-spelling records under counted rejection reasons — excluded, never merged. The crosswalk-observation tier — labels observed against entities on accepted Seshat-coded rows — carries an additional name-coherence guard on the HCED label pass only: the observed entity's own name, aliases, or Wikipedia titles must contain the label, because observations are unapproved crosswalk pairings and a single mis-pairing would otherwise satisfy the uniqueness gate by the error. The IWD resolver deliberately does not adopt this guard, so the IWD aggregation output stays pinned by its own promotion rules rather than depending on the HCED pass. Label-resolved rows never feed the observation store: doing so would let one resolution manufacture uniqueness for later labels, make results depend on file row order, and change the committed IWD promotion — only Seshat-crosswalk-coded, first-pass-accepted rows ever write observations. Label-resolved events carry a declared lower confidence tier than crosswalk-resolved events (one identity-confidence step per label-resolved side) and an `identity_resolution` provenance marker recording the resolution tier of each side; its absence on earlier events means crosswalk resolution.

IWBD battle rows are promoted as tactical engagements under a declared ruleset of the same shape. A battle enters only when it is not a duplicate of any curated seed event, of any of the 8,881 HCED candidates — promoted or staged, because a row rejected today may promote later and no event may ever be able to enter the ledger twice — or of an earlier IWBD row, matched by normalized battle name and year with a ±1-year tolerance; 1,189 IWBD records match an HCED candidate this way and are excluded as presumptive duplicates, with the HCED side always winning and no HCED record modified. A row whose date span strictly contains a differently-named battle of the same war is staged as a presumptive campaign umbrella so that only HCED-consistent battle-level granularity enters; the rule is deliberately over-inclusive, so a genuinely distinct long engagement that merely brackets a shorter same-war sibling by date is also staged — ambiguity always fails, and exclusion only costs provisional yield. A row whose candidate id fails the staged `iwbd-{war}-{iwd}-{row}` format is rejected under its own declared counter (measured zero on the current snapshot) and cannot participate in containment comparisons. The coded victor must be one of the two named sides with an agreeing attacker/defender role: IWBD's coded `inconclusive` is a real source assertion of no tactical decision at the attacker's objective and maps to a tactical stalemate, while a blank or mismatched victor is rejected, never coerced. The war-level victor code is ignored entirely — it describes the enclosing war's outcome, a different evidence layer, and battle records never update strategic outcomes. Coalition and composite side labels ("Allies", "Central Powers", slash/comma/parenthesis composites) stay staged pending curated per-war coalition composition, and both sides must resolve to unique time-bounded identities outside declared deny windows (the label "Turkey" in 1920–1923 is denied outright, so Kemalist-era battles can never attach to the Ottoman Empire). Because IWBD carries day-precision dates but no per-battle magnitude data, accepted events use `date_precision: "day"`, a flat declared confidence, and declared uniform battle-level scale and stakes defaults, with severity capped at limited by construction.
