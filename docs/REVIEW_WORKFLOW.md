...
- Wave 8 promotes 46 HCED rows only through candidate-keyed exact contracts: 9 in the African-states lane, 15 in New Zealand, and 22 in North America. Each contract pins the queue-row fingerprint, event key, dates, sides, time-bounded entity IDs, and evidence set. It creates no generic alias and permits no umbrella ethnonym inheritance. Four exact contracts use documented source-backed outcome overrides; massacre/noncompetitive records and rows with unresolved actors, coalitions, or outcomes remain explicit fingerprinted holds.
