...
The 1,464-source registry contains 867 sources carrying the direct outcome role
across 788 dependency families. Current event-level contracts select 756 of
those sources across 691 families. This maps 5,376 of 5,416 rated events:
5,152 HCED encounters, 64 IWD parent wars, 153 IWBD battles, and 7 UCDP
termination episodes. Of the mapped events, 5,017 cite one declared direct
outcome family and 359 cite between two and six. Family cardinality is an audit
...
For the 5,152 rated HCED events, the release-to-candidate join is an
exact bijection: 1,887 crosswalk-resolved events, 2,481 label-resolved events,
and 784 candidate-keyed reviewed events: 76 from Wave 6, 192 from Wave 7, and
516 from Wave 8, with no missing, ambiguous, or colliding bindings.
...
country. After fail-closed quarantine, 4,795 events carry an exact
source-transcribed Point, 5,057 carry the source's modern
country/geographic-jurisdiction string, and 5,106 carry at least one location
...
- 2,373 time-bounded polity identities in the rated-and-unrated registry;
- 1,025 release entity records, of which 1,017 distinct IDs actually participate
  in rated events;
- 1,464 registered provenance sources across 1,199 source families; and
- 5,416 rating events: 40 manually curated events, 1,887 crosswalk-resolved,
  2,481 label-resolved, and 784 candidate-keyed HCED tactical encounters
  (76 Wave 6 + 192 Wave 7 + 516 Wave 8),
...
candidate IDs already published in the ledger before ranking and reports 2,221
touched deferred rows, 2,197 unresolved normalized labels, and 1,006
...
superseded source envelope. None receives a rating or counts among the 1,017
participant IDs.
...
review now produces the current measured 2,373-row registry; every release
...
candidates, 36,932 remain outside the rating ledger because their layer,
...
Exactly 5,376 rated events have an explicit direct-outcome mapping; the
remaining 40 are the curated seed events and stay unknown pending claim-level
locator review. Of the mapped events, 5,017 cite one declared direct-outcome
family and 359 cite more than one.
