"""Fail-closed candidate audit of HCED's two First Carlist War Bilbao rows.

``Christinos`` is resolved only for the two fingerprinted HCED candidates and
never becomes a generic alias.  Both contracts reuse the existing,
conflict-bounded Isabeline and Carlist identities.  They rate the failure of
the Carlist forces to take Bilbao; foreign assistance and the place-like raw
labels ``Navarre`` and ``Bilbao`` are not promoted into participant entities.

The two matching Wikidata sieges and their nested Battle of Luchana record are
discovery-only.  Their absent winners remain unknown and are never converted
to draws or used to originate an independently rated event.
"""

from __future__ import annotations

import hashlib
import json
from collections import Counter
from typing import Any, Iterable, Mapping

from .common import _slug, normalize_label
from .wave7_global import canonical_hced_row_sha256
from .wave8_exact import (
    expected_exact_hced_win_participants,
    install_exact_entities,
    install_exact_sources,
    promote_exact_hced_contracts,
    validate_exact_hced_inventory,
)


__all__ = (
    "WAVE8_BILBAO_CONTRACT_IDS",
    "WAVE8_BILBAO_CONTRACTS",
    "WAVE8_BILBAO_COUNTRY_QUARANTINE_ADDITIONS",
    "WAVE8_BILBAO_DISCOVERY_EXPECTED",
    "WAVE8_BILBAO_DISCOVERY_NESTED",
    "WAVE8_BILBAO_DISCOVERY_ROW_HASHES",
    "WAVE8_BILBAO_DISCOVERY_TWINS",
    "WAVE8_BILBAO_ENTITIES",
    "WAVE8_BILBAO_EXPECTED_CANDIDATE_IDS",
    "WAVE8_BILBAO_FINAL_AUDIT_SIGNATURE",
    "WAVE8_BILBAO_FUNNEL_AUDIT",
    "WAVE8_BILBAO_HOLDS",
    "WAVE8_BILBAO_INTEGRATION_DISPOSITIONS",
    "WAVE8_BILBAO_LOCATION_QUARANTINE_REASONS",
    "WAVE8_BILBAO_POINT_QUARANTINE_ADDITIONS",
    "WAVE8_BILBAO_RESERVED_IDS",
    "WAVE8_BILBAO_ROW_HASHES",
    "WAVE8_BILBAO_SOURCES",
    "install_wave8_bilbao_entities",
    "install_wave8_bilbao_sources",
    "promote_wave8_bilbao_contracts",
    "validate_wave8_bilbao_current_artifact_state",
    "validate_wave8_bilbao_discovery_dispositions",
    "validate_wave8_bilbao_funnel",
    "validate_wave8_bilbao_integration_dispositions",
    "validate_wave8_bilbao_queue_contracts",
    "wave8_bilbao_audit_signature",
    "wave8_bilbao_cohort_counts",
    "wave8_bilbao_counts",
    "wave8_bilbao_metadata",
)


_LANE_NAME = "Wave 8 exact Bilbao siege audit"
_MODULE_OWNER = "military_elo.promotion.wave8_bilbao"
_EVENT_ID_PREFIX = "hced_wave8_bilbao_"
_EXACT_LABEL = "christinos"
_COHORT = "first_carlist_war_bilbao_sieges_1835_1836"

_ISABELINE_FORCES = "isabeline_government_forces_first_carlist_war"
_CARLIST_FORCES = "carlist_army_first_war"


def _source(
    source_id: str,
    title: str,
    url: str,
    publisher: str,
    source_type: str,
    source_family_id: str,
) -> dict[str, Any]:
    return {
        "id": source_id,
        "title": title,
        "url": url,
        "publisher": publisher,
        "license": "Citation and link only",
        "source_type": source_type,
        "accessed": "2026-07-21",
        "source_family_id": source_family_id,
        "evidence_roles": [
            "identity_boundary_or_context_reference",
            "outcome",
            "outcome_consistency_crosscheck",
        ],
    }


WAVE8_BILBAO_SOURCES: tuple[dict[str, Any], ...] = (
    _source(
        "wave8_bilbao_zumalakarregi_museum_sieges",
        "Bilbao (VI-1835 y X/XII-1836)",
        (
            "https://www.zumalakarregimuseoa.eus/es/actividades/"
            "investigacion-y-documentacion/historia-del-siglo-xix-en-el-"
            "pais-vasco/batallas-y-acciones/bilbao-vi-1835-y-x-xii-1836"
        ),
        "Museo Zumalakarregi Museoa",
        "official_museum_history",
        "zumalakarregi_museum_bilbao_sieges",
    ),
    _source(
        "wave8_bilbao_rah_zumalacarregui_timeline",
        "Tomás de Zumalacárregui e Imaz: hechos y lugares",
        (
            "https://historia-hispanica.rah.es/biografias/"
            "45567-tomas-de-zumalacarregui-e-imaz"
        ),
        "Real Academia de la Historia",
        "scholarly_national_biography_timeline",
        "rah_historia_hispanica_zumalacarregui",
    ),
    _source(
        "wave8_bilbao_ferra_monge_first_siege",
        "La perspectiva carlista del primer sitio de Bilbao en 1835",
        "https://dialnet.unirioja.es/descarga/articulo/10558617.pdf",
        "kult-ur / Universitat Jaume I via Dialnet",
        "peer_reviewed_history_article",
        "ferra_monge_bilbao_1835_2026",
    ),
    _source(
        "wave8_bilbao_army_museum_espartero",
        "El Espadón de Espartero: un regalo de Vizcaya",
        (
            "https://ejercito.defensa.gob.es/museo/comun/"
            "EL_MUSEO_DEL_EJERCITO_ES_NOTICIA/piezas/"
            "2023.05.23_EL_ESPADON_DE_ESPARTERO"
        ),
        "Museo del Ejército, Ministerio de Defensa de España",
        "official_military_museum_history",
        "spanish_army_museum_espartero_sword",
    ),
)

_SOURCE_BY_ID = {str(source["id"]): source for source in WAVE8_BILBAO_SOURCES}


# Both participants are already curated and conflict-bounded.  An empty entity
# tranche guarantees that Bilbao cannot create an alias, a place identity, or a
# broad Spanish-state continuity shortcut.
WAVE8_BILBAO_ENTITIES: tuple[dict[str, Any], ...] = ()


WAVE8_BILBAO_ROW_HASHES: dict[str, str] = {
    "hced-Bilbao1835-1": (
        "be23b58ce654ee3fff6f1ba4ae945ac9e8e63d36c6d8299de0c9a0813774f76a"
    ),
    "hced-Bilbao1836-1": (
        "2092772aa3f1d7676b009c7debe4988bbba90a7c5afa7f80740371a1d97a9789"
    ),
}
WAVE8_BILBAO_EXPECTED_CANDIDATE_IDS = frozenset(WAVE8_BILBAO_ROW_HASHES)


WAVE8_BILBAO_FUNNEL_AUDIT: dict[str, Any] = {
    "candidate_ids": [],
    "event_candidate_id_sha256": (
        "624fa36738091da76baaa86c372e3288ea013e75a5730a62de53beba4f393264"
    ),
    "events_touched": 2,
    "failure_cases": {
        "multiple_time_valid_candidates": 0,
        "one_wrong_interval_candidate": 0,
        "policy_denied_window": 0,
        "resolver_guard_or_tier_conflict": 0,
        "zero_time_valid_candidates": 2,
    },
    "label": _EXACT_LABEL,
    "rated_counterpart_entities": 1,
    "sole_blocker_events": 2,
    "time_valid_candidate_ids": [],
    "unresolved_side_attempts": 2,
}


def _canonical(
    name: str,
    year: int,
    date_text: str,
    start_date: str,
    end_date: str,
) -> dict[str, Any]:
    return {
        "canonical_key": f"{_slug(name)}:{year}:{year}",
        "date_precision": "day",
        "date_text": date_text,
        "start_date": start_date,
        "end_date": end_date,
        "granularity": "single_siege_in_the_first_carlist_war",
        "name": name,
        "year_low": year,
        "year_high": year,
    }


_EVENT_SOURCE_IDS: dict[str, tuple[str, ...]] = {
    "hced-Bilbao1835-1": (
        "wave8_bilbao_ferra_monge_first_siege",
        "wave8_bilbao_rah_zumalacarregui_timeline",
        "wave8_bilbao_zumalakarregi_museum_sieges",
    ),
    "hced-Bilbao1836-1": (
        "wave8_bilbao_army_museum_espartero",
        "wave8_bilbao_zumalakarregi_museum_sieges",
    ),
}


_EVENT_EVIDENCE_ROLES: dict[str, dict[str, list[str]]] = {
    "hced-Bilbao1835-1": {
        "wave8_bilbao_ferra_monge_first_siege": [
            "first_siege_granularity_crosscheck",
            "tactical_outcome_crosscheck",
        ],
        "wave8_bilbao_rah_zumalacarregui_timeline": [
            "exact_siege_interval_crosscheck",
            "tactical_outcome_crosscheck",
        ],
        "wave8_bilbao_zumalakarregi_museum_sieges": [
            "exact_belligerents",
            "exact_siege_interval",
            "siege_granularity",
            "tactical_outcome",
        ],
    },
    "hced-Bilbao1836-1": {
        "wave8_bilbao_army_museum_espartero": [
            "relief_date_crosscheck",
            "tactical_outcome_crosscheck",
        ],
        "wave8_bilbao_zumalakarregi_museum_sieges": [
            "exact_belligerents",
            "exact_siege_interval",
            "siege_granularity",
            "tactical_outcome",
        ],
    },
}


def _contract(
    candidate_id: str,
    canonical_event: Mapping[str, Any],
    audit_note: str,
    *,
    confidence: float,
) -> dict[str, Any]:
    outcomes = sorted(_EVENT_SOURCE_IDS[candidate_id])
    return {
        "raw_row_sha256": WAVE8_BILBAO_ROW_HASHES[candidate_id],
        "canonical_event": dict(canonical_event),
        "cohort": _COHORT,
        "side_1_entity_ids": [_ISABELINE_FORCES],
        "side_2_entity_ids": [_CARLIST_FORCES],
        "disposition": "promote",
        "result_type": "win",
        "winner_side": 1,
        "war_type": "civil_war",
        "confidence": confidence,
        "evidence_refs": outcomes,
        "outcome_source_ids": outcomes,
        "outcome_source_family_ids": sorted(
            {
                str(_SOURCE_BY_ID[source_id]["source_family_id"])
                for source_id in outcomes
            }
        ),
        "event_evidence_roles": {
            source_id: list(roles)
            for source_id, roles in sorted(
                _EVENT_EVIDENCE_ROLES[candidate_id].items()
            )
        },
        "date_source_ids": outcomes,
        "source_date_refinement": True,
        "source_outcome_override": False,
        "outcome_reversal": False,
        "actor_override": "candidate_keyed_exact_first_carlist_war_factions",
        "expected_scale_level": 2,
        "duplicate_ownership": {
            "owner_module": _MODULE_OWNER,
            "status": "canonical_hced_owner",
        },
        "audit_note": audit_note,
    }


WAVE8_BILBAO_CONTRACTS: dict[str, dict[str, Any]] = {
    "hced-Bilbao1835-1": _contract(
        "hced-Bilbao1835-1",
        _canonical(
            "First Siege of Bilbao",
            1835,
            "10 June-1 July 1835",
            "1835-06-10",
            "1835-07-01",
        ),
        (
            "The museum chronology and Real Academia timeline independently pin "
            "the Carlist blockade from 10 June until Eraso lifted it on 1 July; "
            "the peer-reviewed siege study confirms the bounded operation. Only "
            "the Isabeline-versus-Carlist tactical result is rated. Foreign naval "
            "assistance, Navarre, and Bilbao are not separate participant polities."
        ),
        confidence=0.95,
    ),
    "hced-Bilbao1836-1": _contract(
        "hced-Bilbao1836-1",
        _canonical(
            "Second Siege of Bilbao",
            1836,
            "23 October-25 December 1836",
            "1836-10-23",
            "1836-12-25",
        ),
        (
            "The museum chronology pins the renewed Carlist siege from 23 October "
            "through its failure at Christmas, while the Spanish Army Museum "
            "independently records Espartero's relief of Bilbao on 24-25 December. "
            "The nested Battle of Luchana is not rated separately. Only the two "
            "candidate sides are bound; foreign assistance is not converted into "
            "an additional polity participant."
        ),
        confidence=0.95,
    ),
}


WAVE8_BILBAO_HOLDS: dict[str, dict[str, Any]] = {}
WAVE8_BILBAO_CONTRACT_IDS = frozenset(WAVE8_BILBAO_CONTRACTS)
WAVE8_BILBAO_RESERVED_IDS = WAVE8_BILBAO_CONTRACT_IDS


# The HCED point is an appropriate city-level representation for each full
# siege, and the source country is a location assertion only.  This lane adds
# no location quarantine.
WAVE8_BILBAO_POINT_QUARANTINE_ADDITIONS: frozenset[str] = frozenset()
WAVE8_BILBAO_COUNTRY_QUARANTINE_ADDITIONS: frozenset[str] = frozenset()
WAVE8_BILBAO_LOCATION_QUARANTINE_REASONS: dict[str, dict[str, Any]] = {}


WAVE8_BILBAO_DISCOVERY_TWINS: dict[str, str] = {
    "hced-Bilbao1835-1": "Q3755661",
    "hced-Bilbao1836-1": "Q3756025",
}
WAVE8_BILBAO_DISCOVERY_NESTED: dict[str, str] = {
    "Q3310734": "hced-Bilbao1836-1",
}
WAVE8_BILBAO_DISCOVERY_ROW_HASHES: dict[str, str] = {
    "Q3310734": (
        "354ab246258dc63bd02b339e496ec81d64eff853e2a25f0557c97655fd2d6009"
    ),
    "Q3755661": (
        "0ca9b4a056bfffe11ad7bd7e846acf5e678dc1b435b0decd16c7bced52807687"
    ),
    "Q3756025": (
        "73a6867b1e9031406a7e6784ea9cc15b48fd4e5bf175e726d5a5557bd85f3395"
    ),
}
WAVE8_BILBAO_DISCOVERY_EXPECTED: dict[str, dict[str, Any]] = {
    "Q3310734": {
        "date": "1836-12-23T00:00:00Z",
        "end_date": "1836-12-24T00:00:00Z",
        "kind": "engagement",
        "name": "Battle of Luchana",
        "outcome_disposition": "unknown_never_draw",
        "relationship": "nested_constituent_of_second_siege",
        "participant_labels": ["Carlist army", "Christinos"],
    },
    "Q3755661": {
        "date": "1835-06-10T00:00:00Z",
        "end_date": "1835-07-01T00:00:00Z",
        "kind": "siege",
        "name": "Siege of Bilbao",
        "outcome_disposition": "unknown_never_draw",
        "relationship": "exact_discovery_duplicate",
        "participant_labels": ["Carlist army", "Christinos"],
    },
    "Q3756025": {
        "date": "1836-10-23T00:00:00Z",
        "end_date": "1836-12-25T00:00:00Z",
        "kind": "siege",
        "name": "Siege of Bilbao",
        "outcome_disposition": "unknown_never_draw",
        "relationship": "exact_discovery_duplicate",
        "participant_labels": ["Auxiliary Legion", "Carlist army", "Christinos"],
    },
}
WAVE8_BILBAO_INTEGRATION_DISPOSITIONS: dict[str, dict[str, Any]] = {
    candidate_id: {
        "source_dataset": "wikidata-battles",
        "disposition": (
            "discovery_only_nested_constituent"
            if candidate_id in WAVE8_BILBAO_DISCOVERY_NESTED
            else "discovery_only_duplicate"
        ),
        "hced_candidate_id": (
            WAVE8_BILBAO_DISCOVERY_NESTED.get(candidate_id)
            or next(
                hced_id
                for hced_id, discovery_id in WAVE8_BILBAO_DISCOVERY_TWINS.items()
                if discovery_id == candidate_id
            )
        ),
        "outcome_disposition": WAVE8_BILBAO_DISCOVERY_EXPECTED[candidate_id][
            "outcome_disposition"
        ],
    }
    for candidate_id in sorted(WAVE8_BILBAO_DISCOVERY_ROW_HASHES)
}


def _canonical_json(value: Any) -> str:
    return json.dumps(value, ensure_ascii=False, sort_keys=True, separators=(",", ":"))


def _signature_payload() -> dict[str, Any]:
    return {
        "contracts": WAVE8_BILBAO_CONTRACTS,
        "discovery_expected": WAVE8_BILBAO_DISCOVERY_EXPECTED,
        "discovery_nested": WAVE8_BILBAO_DISCOVERY_NESTED,
        "discovery_row_hashes": WAVE8_BILBAO_DISCOVERY_ROW_HASHES,
        "discovery_twins": WAVE8_BILBAO_DISCOVERY_TWINS,
        "entities": WAVE8_BILBAO_ENTITIES,
        "funnel": WAVE8_BILBAO_FUNNEL_AUDIT,
        "holds": WAVE8_BILBAO_HOLDS,
        "integration_dispositions": WAVE8_BILBAO_INTEGRATION_DISPOSITIONS,
        "location_reasons": WAVE8_BILBAO_LOCATION_QUARANTINE_REASONS,
        "row_hashes": WAVE8_BILBAO_ROW_HASHES,
        "sources": WAVE8_BILBAO_SOURCES,
    }


def wave8_bilbao_audit_signature() -> str:
    return hashlib.sha256(
        _canonical_json(_signature_payload()).encode("utf-8")
    ).hexdigest()


WAVE8_BILBAO_FINAL_AUDIT_SIGNATURE = (
    "ea3b38fbd6d6ff79e3246b457cd41d483f58688ac69f9b868f442ba0cd0c7ef2"
)


def _is_sorted_unique(values: Iterable[str]) -> bool:
    items = list(map(str, values))
    return items == sorted(set(items))


def _validate_static() -> None:
    source_ids = set(_SOURCE_BY_ID)
    if len(source_ids) != len(WAVE8_BILBAO_SOURCES):
        raise ValueError(f"{_LANE_NAME} duplicate source id")
    if WAVE8_BILBAO_ENTITIES:
        raise ValueError(f"{_LANE_NAME} must reuse existing participant identities")
    if WAVE8_BILBAO_RESERVED_IDS != WAVE8_BILBAO_EXPECTED_CANDIDATE_IDS:
        raise ValueError(f"{_LANE_NAME} disposition inventory drift")
    if WAVE8_BILBAO_HOLDS:
        raise ValueError(f"{_LANE_NAME} unexpected hold inventory")
    if (
        WAVE8_BILBAO_POINT_QUARANTINE_ADDITIONS
        or WAVE8_BILBAO_COUNTRY_QUARANTINE_ADDITIONS
        or WAVE8_BILBAO_LOCATION_QUARANTINE_REASONS
    ):
        raise ValueError(f"{_LANE_NAME} location retention policy drift")

    expected_urls = {
        "wave8_bilbao_army_museum_espartero": (
            "https://ejercito.defensa.gob.es/museo/comun/"
            "EL_MUSEO_DEL_EJERCITO_ES_NOTICIA/piezas/"
            "2023.05.23_EL_ESPADON_DE_ESPARTERO"
        ),
        "wave8_bilbao_ferra_monge_first_siege": (
            "https://dialnet.unirioja.es/descarga/articulo/10558617.pdf"
        ),
        "wave8_bilbao_rah_zumalacarregui_timeline": (
            "https://historia-hispanica.rah.es/biografias/"
            "45567-tomas-de-zumalacarregui-e-imaz"
        ),
        "wave8_bilbao_zumalakarregi_museum_sieges": (
            "https://www.zumalakarregimuseoa.eus/es/actividades/"
            "investigacion-y-documentacion/historia-del-siglo-xix-en-el-"
            "pais-vasco/batallas-y-acciones/bilbao-vi-1835-y-x-xii-1836"
        ),
    }
    for source_id, source in _SOURCE_BY_ID.items():
        if source["url"] != expected_urls[source_id]:
            raise ValueError(f"{_LANE_NAME} canonical source URL drift: {source_id}")
        if not _is_sorted_unique(source["evidence_roles"]):
            raise ValueError(f"{_LANE_NAME} source role order drift: {source_id}")
        if "outcome" not in source["evidence_roles"]:
            raise ValueError(f"{_LANE_NAME} non-outcome source used for result")

    expected = {
        "hced-Bilbao1835-1": (
            "First Siege of Bilbao",
            1835,
            "10 June-1 July 1835",
            "1835-06-10",
            "1835-07-01",
            0.95,
        ),
        "hced-Bilbao1836-1": (
            "Second Siege of Bilbao",
            1836,
            "23 October-25 December 1836",
            "1836-10-23",
            "1836-12-25",
            0.95,
        ),
    }
    used_sources: set[str] = set()
    for candidate_id, contract in WAVE8_BILBAO_CONTRACTS.items():
        if (
            contract["disposition"] != "promote"
            or contract["result_type"] != "win"
            or contract["winner_side"] != 1
            or contract["source_outcome_override"]
            or contract["outcome_reversal"]
        ):
            raise ValueError(f"{_LANE_NAME} outcome disposition drift: {candidate_id}")
        if (
            contract["side_1_entity_ids"],
            contract["side_2_entity_ids"],
        ) != ([_ISABELINE_FORCES], [_CARLIST_FORCES]):
            raise ValueError(f"{_LANE_NAME} exact actor drift: {candidate_id}")

        canonical = contract["canonical_event"]
        name, year, date_text, start_date, end_date, confidence = expected[candidate_id]
        if (
            canonical["canonical_key"] != f"{_slug(name)}:{year}:{year}"
            or canonical["name"] != name
            or canonical["date_precision"] != "day"
            or canonical["date_text"] != date_text
            or (canonical["start_date"], canonical["end_date"])
            != (start_date, end_date)
            or (canonical["year_low"], canonical["year_high"]) != (year, year)
            or canonical["granularity"]
            != "single_siege_in_the_first_carlist_war"
            or float(contract["confidence"]) != confidence
            or int(contract["expected_scale_level"]) != 2
        ):
            raise ValueError(f"{_LANE_NAME} date/granularity drift: {candidate_id}")

        outcomes = list(map(str, contract["outcome_source_ids"]))
        evidence = list(map(str, contract["evidence_refs"]))
        date_sources = list(map(str, contract["date_source_ids"]))
        expected_sources = sorted(_EVENT_SOURCE_IDS[candidate_id])
        expected_families = sorted(
            str(_SOURCE_BY_ID[source_id]["source_family_id"])
            for source_id in expected_sources
        )
        if (
            outcomes != expected_sources
            or evidence != outcomes
            or date_sources != outcomes
            or contract["outcome_source_family_ids"] != expected_families
            or len(expected_families) < 2
            or contract["source_date_refinement"] is not True
        ):
            raise ValueError(f"{_LANE_NAME} source closure drift: {candidate_id}")
        role_map = contract["event_evidence_roles"]
        if set(role_map) != set(expected_sources):
            raise ValueError(f"{_LANE_NAME} event source-role closure drift")
        for source_id, roles in role_map.items():
            if not _is_sorted_unique(roles) or not roles:
                raise ValueError(
                    f"{_LANE_NAME} event-specific role drift: "
                    f"{candidate_id}/{source_id}"
                )
        used_sources.update(outcomes)

    if used_sources != source_ids:
        raise ValueError(f"{_LANE_NAME} source fixtures are not exactly consumed")
    if set(WAVE8_BILBAO_DISCOVERY_TWINS) != WAVE8_BILBAO_CONTRACT_IDS:
        raise ValueError(f"{_LANE_NAME} discovery owner drift")
    if set(WAVE8_BILBAO_DISCOVERY_TWINS.values()) | set(
        WAVE8_BILBAO_DISCOVERY_NESTED
    ) != set(WAVE8_BILBAO_DISCOVERY_ROW_HASHES):
        raise ValueError(f"{_LANE_NAME} discovery inventory drift")
    if set(WAVE8_BILBAO_DISCOVERY_EXPECTED) != set(
        WAVE8_BILBAO_DISCOVERY_ROW_HASHES
    ):
        raise ValueError(f"{_LANE_NAME} discovery expectation drift")
    if any(
        item["outcome_disposition"] != "unknown_never_draw"
        for item in WAVE8_BILBAO_DISCOVERY_EXPECTED.values()
    ):
        raise ValueError(f"{_LANE_NAME} unknown-is-never-draw guard drift")
    if set(WAVE8_BILBAO_INTEGRATION_DISPOSITIONS) != set(
        WAVE8_BILBAO_DISCOVERY_ROW_HASHES
    ):
        raise ValueError(f"{_LANE_NAME} integration disposition drift")
    if wave8_bilbao_audit_signature() != WAVE8_BILBAO_FINAL_AUDIT_SIGNATURE:
        raise ValueError(f"{_LANE_NAME} final audit signature changed")


def validate_wave8_bilbao_queue_contracts(
    hced_rows: list[dict[str, Any]],
) -> dict[str, int]:
    _validate_static()
    exact = [
        row
        for row in hced_rows
        if normalize_label(row.get("side_1_raw")) == _EXACT_LABEL
        or normalize_label(row.get("side_2_raw")) == _EXACT_LABEL
    ]
    exact_ids = {str(row.get("candidate_id")) for row in exact}
    if exact_ids != WAVE8_BILBAO_RESERVED_IDS or len(exact) != len(exact_ids):
        raise ValueError(f"{_LANE_NAME} exact-label inventory changed")
    by_id = {str(row["candidate_id"]): row for row in exact}
    expected_years = {
        "hced-Bilbao1835-1": 1835,
        "hced-Bilbao1836-1": 1836,
    }
    for candidate_id, expected_hash in WAVE8_BILBAO_ROW_HASHES.items():
        row = by_id[candidate_id]
        if canonical_hced_row_sha256(row) != expected_hash:
            raise ValueError(f"{_LANE_NAME} row fingerprint changed: {candidate_id}")
        if (
            row.get("side_1_raw") != "Christinos"
            or row.get("side_2_raw") != "Carlists"
            or row.get("winner_raw") != "Christinos"
            or row.get("loser_raw") != "Carlists"
            or row.get("winner_loser_complete") is not True
            or row.get("massacre_raw") != "No"
            or row.get("do_not_rate_automatically") is not True
            or row.get("duplicate_source_id") is not False
        ):
            raise ValueError(f"{_LANE_NAME} locked outcome/actor row drift: {candidate_id}")
        year = expected_years[candidate_id]
        if (row.get("year_low"), row.get("year_best"), row.get("year_high")) != (
            year,
            year,
            year,
        ):
            raise ValueError(f"{_LANE_NAME} locked year drift: {candidate_id}")

    counts = validate_exact_hced_inventory(
        hced_rows,
        WAVE8_BILBAO_CONTRACTS,
        WAVE8_BILBAO_HOLDS,
        lane_name=_LANE_NAME,
    )
    return {**counts, "exact_label_rows": len(exact)}


def validate_wave8_bilbao_funnel(funnel: Mapping[str, Any]) -> dict[str, int]:
    _validate_static()
    rows = [row for row in funnel.get("labels", []) if row.get("label") == _EXACT_LABEL]
    if len(rows) != 1:
        raise ValueError(f"{_LANE_NAME} expected one funnel label")
    row = rows[0]
    actual = {
        "candidate_ids": list(map(str, row.get("candidate_ids", []))),
        "event_candidate_id_sha256": str(row.get("event_candidate_id_sha256")),
        "events_touched": int(row.get("events_touched", -1)),
        "failure_cases": {
            key: int(row.get("failure_cases", {}).get(key, -1))
            for key in WAVE8_BILBAO_FUNNEL_AUDIT["failure_cases"]
        },
        "label": str(row.get("label")),
        "rated_counterpart_entities": int(row.get("rated_counterpart_entities", -1)),
        "sole_blocker_events": int(row.get("sole_blocker_events", -1)),
        "time_valid_candidate_ids": list(
            map(str, row.get("time_valid_candidate_ids", []))
        ),
        "unresolved_side_attempts": int(row.get("unresolved_side_attempts", -1)),
    }
    if actual != WAVE8_BILBAO_FUNNEL_AUDIT:
        raise ValueError(f"{_LANE_NAME} funnel pin changed: {actual}")
    return {
        "events_touched": actual["events_touched"],
        "sole_blocker_events": actual["sole_blocker_events"],
        "unresolved_side_attempts": actual["unresolved_side_attempts"],
        "zero_time_valid_candidates": actual["failure_cases"][
            "zero_time_valid_candidates"
        ],
    }


def _full_row_sha256(row: Mapping[str, Any]) -> str:
    return hashlib.sha256(_canonical_json(dict(row)).encode("utf-8")).hexdigest()


def validate_wave8_bilbao_discovery_dispositions(
    wikidata_rows: list[dict[str, Any]],
) -> dict[str, int]:
    _validate_static()
    by_id: dict[str, list[dict[str, Any]]] = {}
    for row in wikidata_rows:
        by_id.setdefault(str(row.get("candidate_id")), []).append(row)
    for candidate_id, expected_hash in WAVE8_BILBAO_DISCOVERY_ROW_HASHES.items():
        rows = by_id.get(candidate_id, [])
        if len(rows) != 1:
            raise ValueError(
                f"{_LANE_NAME} discovery row {candidate_id} expected once, "
                f"found {len(rows)}"
            )
        row = rows[0]
        if _full_row_sha256(row) != expected_hash:
            raise ValueError(f"{_LANE_NAME} discovery fingerprint changed: {candidate_id}")
        expected = WAVE8_BILBAO_DISCOVERY_EXPECTED[candidate_id]
        participant_labels = sorted(
            str(participant.get("label"))
            for participant in row.get("participants", [])
        )
        if (
            row.get("source") != "wikidata-battles"
            or row.get("do_not_rate_automatically") is not True
            or row.get("name") != expected["name"]
            or row.get("date") != expected["date"]
            or row.get("end_date") != expected["end_date"]
            or row.get("kind") != expected["kind"]
            or participant_labels != expected["participant_labels"]
            or row.get("winners") != []
        ):
            raise ValueError(
                f"{_LANE_NAME} discovery non-rating guard changed: {candidate_id}"
            )
    return {
        "discovery_nonrating_records": len(WAVE8_BILBAO_DISCOVERY_ROW_HASHES),
        "discovery_promotions": 0,
        "nested_constituents": len(WAVE8_BILBAO_DISCOVERY_NESTED),
        "unknown_never_draw_rows": len(WAVE8_BILBAO_DISCOVERY_ROW_HASHES),
    }


def _row_year(row: Mapping[str, Any]) -> int | None:
    for key in (
        "year",
        "year_best",
        "year_low",
        "start_year",
        "year_start",
        "batyear",
    ):
        try:
            if row.get(key) is not None:
                return int(row[key])
        except (TypeError, ValueError):
            pass
    for key in ("start_date", "date"):
        value = str(row.get(key) or "")
        if len(value) >= 4 and value[:4].isdigit():
            return int(value[:4])
    return None


def _row_names(row: Mapping[str, Any]) -> set[str]:
    values = [
        row.get("name"),
        row.get("battle_name"),
        row.get("batname"),
        row.get("event_name"),
        row.get("war_name"),
    ]
    aliases = row.get("aliases", []) or []
    if isinstance(aliases, str):
        values.append(aliases)
    else:
        values.extend(aliases)
    return {normalize_label(value) for value in values if normalize_label(value)}


_EVENT_ALIASES: dict[str, set[str]] = {
    "hced-Bilbao1835-1": {
        "Bilbao",
        "First Siege of Bilbao",
        "Siege of Bilbao",
    },
    "hced-Bilbao1836-1": {
        "Battle of Luchana",
        "Bilbao",
        "Luchana",
        "Second Siege of Bilbao",
        "Siege of Bilbao",
    },
}
_DUPLICATE_MATCH_KEYS = frozenset(
    (
        int(WAVE8_BILBAO_CONTRACTS[candidate_id]["canonical_event"]["year_low"]),
        normalize_label(alias),
    )
    for candidate_id, aliases in _EVENT_ALIASES.items()
    for alias in aliases
)
_BILBAO_NAMES = frozenset(
    normalize_label(alias) for aliases in _EVENT_ALIASES.values() for alias in aliases
)


def _is_probable_twin(row: Mapping[str, Any]) -> bool:
    names = _row_names(row)
    matched_names = names & _BILBAO_NAMES
    if not matched_names:
        return False
    year = _row_year(row)
    if year is None:
        return True
    return any((year, name) in _DUPLICATE_MATCH_KEYS for name in matched_names)


def validate_wave8_bilbao_integration_dispositions(
    hced_rows: list[dict[str, Any]],
    iwd_rows: list[dict[str, Any]],
    iwbd_rows: list[dict[str, Any]],
    existing_events: Iterable[Mapping[str, Any]] = (),
) -> dict[str, int]:
    """Prove that no unreviewed HCED/IWD/IWBD/release twin can double-rate."""

    validate_wave8_bilbao_queue_contracts(hced_rows)
    hced_twins = sorted(
        str(row.get("candidate_id") or "<missing-id>")
        for row in hced_rows
        if str(row.get("candidate_id")) not in WAVE8_BILBAO_RESERVED_IDS
        and _is_probable_twin(row)
    )
    iwd_twins = sorted(
        str(row.get("candidate_id") or "<missing-id>")
        for row in iwd_rows
        if _is_probable_twin(row)
    )
    iwbd_twins = sorted(
        str(row.get("candidate_id") or "<missing-id>")
        for row in iwbd_rows
        if _is_probable_twin(row)
    )

    existing = list(existing_events)
    owned = [
        event
        for event in existing
        if event.get("hced_candidate_id") in WAVE8_BILBAO_RESERVED_IDS
        or str(event.get("id", "")).startswith(_EVENT_ID_PREFIX)
    ]
    if owned:
        owned_ids = {str(event.get("hced_candidate_id")) for event in owned}
        if owned_ids != WAVE8_BILBAO_CONTRACT_IDS or len(owned) != len(owned_ids):
            raise ValueError(f"{_LANE_NAME} current release ownership is partial")
        if any(
            not str(event.get("id", "")).startswith(_EVENT_ID_PREFIX)
            for event in owned
        ):
            raise ValueError(f"{_LANE_NAME} current release owner prefix changed")
    release_twins = sorted(
        str(event.get("id") or "<missing-id>")
        for event in existing
        if event not in owned and _is_probable_twin(event)
    )
    if hced_twins or iwd_twins or iwbd_twins or release_twins:
        raise ValueError(
            f"{_LANE_NAME} unreviewed twin(s): hced={hced_twins}, "
            f"iwd={iwd_twins}, iwbd={iwbd_twins}, release={release_twins}"
        )
    return {
        "discovery_nonrating_dispositions": len(
            WAVE8_BILBAO_INTEGRATION_DISPOSITIONS
        ),
        "existing_release_owned_events": len(owned),
        "existing_release_probable_twins": 0,
        "hced_probable_twins": 0,
        "iwd_probable_twins": 0,
        "iwbd_probable_twins": 0,
    }


def install_wave8_bilbao_entities(
    release_entities: dict[str, dict[str, Any]],
) -> None:
    _validate_static()
    install_exact_entities(
        release_entities,
        WAVE8_BILBAO_ENTITIES,
        lane_name=_LANE_NAME,
    )


def install_wave8_bilbao_sources(
    sources_by_id: dict[str, dict[str, Any]],
) -> None:
    _validate_static()
    install_exact_sources(
        sources_by_id,
        WAVE8_BILBAO_SOURCES,
        lane_name=_LANE_NAME,
    )


def promote_wave8_bilbao_contracts(
    hced_rows: list[dict[str, Any]],
    release_entities: Mapping[str, Mapping[str, Any]],
    existing_events: Iterable[Mapping[str, Any]],
) -> list[dict[str, Any]]:
    validate_wave8_bilbao_queue_contracts(hced_rows)
    return promote_exact_hced_contracts(
        hced_rows,
        release_entities,
        existing_events,
        WAVE8_BILBAO_CONTRACTS,
        lane_name=_LANE_NAME,
        event_id_prefix=_EVENT_ID_PREFIX,
    )


def _entity_covers(entity: Mapping[str, Any], low: int, high: int) -> bool:
    start = entity.get("start_year")
    end = entity.get("end_year")
    return (
        start is not None
        and int(start) <= low
        and (end is None or int(end) >= high)
    )


def validate_wave8_bilbao_current_artifact_state(
    release_events: Iterable[Mapping[str, Any]],
    release_entities: Iterable[Mapping[str, Any]],
    release_sources: Iterable[Mapping[str, Any]],
) -> dict[str, Any]:
    """Require Bilbao events and source fixtures to be absent or complete."""

    _validate_static()
    entity_by_id = {str(entity.get("id")): entity for entity in release_entities}
    for entity_id in (_ISABELINE_FORCES, _CARLIST_FORCES):
        entity = entity_by_id.get(entity_id)
        if entity is None or not _entity_covers(entity, 1835, 1836):
            raise ValueError(f"{_LANE_NAME} required identity missing: {entity_id}")
        if list(entity.get("aliases", [])):
            raise ValueError(f"{_LANE_NAME} identity alias inventory changed: {entity_id}")

    events = list(release_events)
    owned = [
        event
        for event in events
        if event.get("hced_candidate_id") in WAVE8_BILBAO_RESERVED_IDS
        or str(event.get("id", "")).startswith(_EVENT_ID_PREFIX)
    ]
    source_ids = set(_SOURCE_BY_ID)
    present_source_ids = {
        str(source.get("id"))
        for source in release_sources
        if str(source.get("id")) in source_ids
    }
    if len(owned) not in {0, len(WAVE8_BILBAO_CONTRACT_IDS)}:
        raise ValueError(f"{_LANE_NAME} current release event inventory is partial")
    if len(present_source_ids) not in {0, len(source_ids)}:
        raise ValueError(f"{_LANE_NAME} current release source inventory is partial")
    if bool(owned) != bool(present_source_ids):
        raise ValueError(f"{_LANE_NAME} current release artifacts are out of sync")
    if not owned:
        return {
            "artifact_state": "absent",
            "installed_sources": 0,
            "promoted_events": 0,
        }

    by_candidate = {str(event.get("hced_candidate_id")): event for event in owned}
    if set(by_candidate) != WAVE8_BILBAO_CONTRACT_IDS or len(owned) != len(
        by_candidate
    ):
        raise ValueError(f"{_LANE_NAME} current release candidate inventory changed")
    for candidate_id, contract in WAVE8_BILBAO_CONTRACTS.items():
        event = by_candidate[candidate_id]
        canonical = contract["canonical_event"]
        expected_id = f"{_EVENT_ID_PREFIX}{_slug(candidate_id, 80)}"
        year = int(canonical["year_low"])
        if (
            event.get("id") != expected_id
            or event.get("name") != canonical["name"]
            or (event.get("year"), event.get("end_year")) != (year, year)
            or event.get("event_type") != "engagement"
            or event.get("date_precision") != "day"
            or event.get("reviewed_granularity")
            != "single_siege_in_the_first_carlist_war"
            or event.get("canonical_event_key") != canonical["canonical_key"]
            or event.get("identity_resolution") != "candidate_keyed_exact"
            or event.get("outcome_source_ids") != contract["outcome_source_ids"]
            or event.get("outcome_source_family_ids")
            != contract["outcome_source_family_ids"]
            or float(event.get("confidence", -1.0)) != float(contract["confidence"])
        ):
            raise ValueError(f"{_LANE_NAME} current release event drift: {candidate_id}")
        expected_participants = expected_exact_hced_win_participants(
            [_ISABELINE_FORCES],
            [_CARLIST_FORCES],
            confidence=float(contract["confidence"]),
            scale_level=int(contract["expected_scale_level"]),
            lane_name=_LANE_NAME,
        )
        if event.get("participants") != expected_participants:
            raise ValueError(
                f"{_LANE_NAME} current release participant drift: {candidate_id}"
            )
        if (
            event.get("geometry")
            != {"type": "Point", "coordinates": [-2.9349852, 43.2630126]}
            or event.get("modern_location_country") != "Spain"
            or "location_provenance" not in event
        ):
            raise ValueError(f"{_LANE_NAME} current release location drift: {candidate_id}")

    return {
        "artifact_state": "integrated",
        "installed_sources": len(present_source_ids),
        "promoted_events": len(owned),
    }


def wave8_bilbao_cohort_counts() -> dict[str, int]:
    _validate_static()
    return dict(
        sorted(
            Counter(
                str(item["cohort"])
                for item in (
                    *WAVE8_BILBAO_CONTRACTS.values(),
                    *WAVE8_BILBAO_HOLDS.values(),
                )
            ).items()
        )
    )


def wave8_bilbao_counts() -> dict[str, int]:
    _validate_static()
    return {
        "country_quarantine_additions": 0,
        "discovery_nonrating_records": len(WAVE8_BILBAO_DISCOVERY_ROW_HASHES),
        "holds": 0,
        "integration_dispositions": len(WAVE8_BILBAO_INTEGRATION_DISPOSITIONS),
        "new_entities": 0,
        "new_sources": len(WAVE8_BILBAO_SOURCES),
        "newly_rated_events": len(WAVE8_BILBAO_CONTRACTS),
        "outcome_overrides": 0,
        "point_quarantine_additions": 0,
        "promotion_contracts": len(WAVE8_BILBAO_CONTRACTS),
        "reviewed_hced_rows": len(WAVE8_BILBAO_RESERVED_IDS),
        "terminal_exclusions": 0,
        "unknown_discovery_outcomes": len(WAVE8_BILBAO_DISCOVERY_ROW_HASHES),
    }


def wave8_bilbao_metadata() -> dict[str, Any]:
    _validate_static()
    return {
        "counts": wave8_bilbao_counts(),
        "cohorts": wave8_bilbao_cohort_counts(),
        "discovery_dispositions": [
            {"candidate_id": candidate_id, **disposition}
            for candidate_id, disposition in sorted(
                WAVE8_BILBAO_INTEGRATION_DISPOSITIONS.items()
            )
        ],
        "final_audit_signature": WAVE8_BILBAO_FINAL_AUDIT_SIGNATURE,
        "promoted_candidate_ids": sorted(WAVE8_BILBAO_CONTRACT_IDS),
    }


_validate_static()
